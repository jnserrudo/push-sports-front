  ...
  return (
    <div className="flex h-screen bg-white text-neutral-900 font-sans overflow-hidden relative">
      ...
      <aside className={`
        ...
        ${isSidebarOpen ? 'w-[85vw] md:w-80 translate-x-0' : 'w-0 -translate-x-full md:w-20 md:translate-x-0'}
      `}>
        
        {/* Branding */}
        <div className="p-8 md:p-14 border-b border-neutral-100 flex flex-col items-center relative">
            {/* Mobile Close Button */}
            <button 
              onClick={() => setIsSidebarOpen(false)}
              className="md:hidden absolute top-4 right-4 p-2 text-neutral-400 hover:text-black transition-colors"
            >
              <X size={20} />
            </button>

            <div className="w-16 h-16 md:w-24 md:h-24 bg-white border-2 border-neutral-100 p-3 md:p-5 rounded-2xl md:rounded-[2rem] shadow-sm mb-6 md:mb-10 transition-transform duration-500">
                <img src="/icono.jpeg" alt="Logo" className="w-full h-full object-contain invert" />
            </div>
            <h1 className="text-xl md:text-3xl font-black tracking-tighter m-0 uppercase text-center leading-none">
               PushSport <br/>
        ...
        <div className="p-4 md:p-8">
            <div className="bg-white p-4 md:p-6 rounded-2xl md:rounded-[2.5rem] flex items-center gap-4 md:gap-6 border-2 md:border-4 border-neutral-100 shadow-soft">
                <div className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-neutral-100 border-2 border-neutral-200 text-neutral-900 flex items-center justify-center font-black text-sm md:text-xl shadow-inner flex-shrink-0">
                    {user?.id_rol === 1 ? 'A' : user?.id_rol === 2 ? 'G' : 'V'}
                </div>
                <div className="flex flex-col flex-1 min-w-0">
                    <span className="font-black text-xs md:text-sm uppercase tracking-wider truncate text-neutral-900">{user?.nombre || 'Personal'}</span>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative bg-white transition-all duration-500">
        ...
        {/* HEADER */}
        <header className="h-20 md:h-24 bg-white border-b-2 md:border-b-4 border-neutral-100 flex items-center justify-between px-6 md:px-10 relative z-50 shadow-sm">
          <div className="flex items-center gap-4 md:gap-6">
            <button 
                onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                className="p-3 md:p-4 bg-neutral-900 border-2 border-neutral-900 text-white hover:bg-black hover:scale-110 active:scale-95 rounded-xl md:rounded-2xl transition-all shadow-xl shadow-neutral-900/10 flex items-center justify-center min-w-[48px] min-h-[48px] md:min-w-[56px] md:min-h-[56px]"
                title={isSidebarOpen ? "Cerrar Panel" : "Abrir Panel"}
            >
              <MenuIcon size={20} md:size={24} className="text-white" />
            </button>
            ...
            <div className="flex items-center gap-3">
                <div className="relative" ref={notificationsRef} onMouseEnter={handleNotificationsEnter} onMouseLeave={handleNotificationsLeave}>
                  <button 
                    ...
                    className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center border-2 transition-all shadow-md hover:scale-105 active:scale-95 ${isNotificationsOpen ? 'border-brand-cyan bg-white' : 'border-neutral-100 bg-white hover:border-neutral-900'}`}
                  >
                      <Bell size={20} md:size={24} className="text-neutral-900" />
                      {unreadCount > 0 && (
                        <div className="absolute top-2 right-2 md:top-3 md:right-3 w-2.5 h-2.5 bg-brand-cyan rounded-full border-2 border-white" />
                      )}
                  </button>
                ...
                <div className="relative" ref={profileRef} onMouseEnter={handleProfileEnter} onMouseLeave={handleProfileLeave}>
                  <button 
                    ...
                    className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center border-2 transition-all shadow-md hover:scale-105 active:scale-95 ${isProfileOpen ? 'border-brand-cyan bg-white' : 'border-neutral-100 bg-white hover:border-neutral-900'}`}
                  >
                      <UserIcon size={20} md:size={24} className="text-neutral-900" />
                  </button>
        ...
        {/* PAGE CONTENT */}
        <main className="flex-1 overflow-y-auto p-4 md:p-10 lg:p-14 bg-neutral-50">
